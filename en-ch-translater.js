...
function setupView() {
        ...
        views: [{
          ...
          events:{
            didBeginEditing: function(sender) {
              ...
              textBeginEdit = sender.text
            },
            didEndEditing: function(sender) {
              if(textBeginEdit !== sender.text) {
  ...
  $ui.render({
    ...
    },
    {
      ...
      props: {
        id: "scrollView",
        scrollEnabled: (isInToday() && getCache("pullToClose", true)),
      },
      layout: function(make, view) {
        make.center.equalTo(view.super)
      ...
      layout: function(make, view) {
        make.centerX.equalTo(view.super)
        make.bottom.equalTo($("card").top).inset((getCache("pullToClose", true)?5:1))
  ...
  if(isInToday()) {
    $widget.height = cardHeight + ((getCache("pullToClose", true))?20:70)
  }
}
...
function setResultInputTool() {
  let number = 3
  let inputView = $("result")
  var toolView = $ui.create({
    type: "view",
    props: {
      frame: $rect(0, 0, 0, 40),
    ...
    views: [{
      ...
      props: {
        ...
        data: [
          {
            title: "界面",
            rows: [tabShowUiItem, tabPullToCloseItem, tabShowColorItem]
          },
          {
            title: "关于",
            rows: array,
...
function solveAction(action) {
  let language = (getSpeechLan() == "英")?"en-US":"zh-CN"
  switch(action) {
    case "inputSpeech":
      $input.speech({
        locale: language,
